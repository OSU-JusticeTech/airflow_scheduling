...
This project includes Alembic configuration (`alembic.ini` and the `alembic/` folder) to manage schema migrations. Use Alembic to apply schema changes to the `justicetech_db` database during initial setup or when updating the schema.

Basic usage (from the project root):

1. Ensure `alembic` is installed (it's included in `requirements.txt`):

```bash
pip install alembic
```

2. Configure the connection URL for Alembic. You can either set it in `alembic.ini` (the `sqlalchemy.url` entry) or export it as an environment variable and update `alembic/env.py` to read it. Example using an environment variable:

```bash
export DATABASE_URL=postgresql://${DB_USER}:${DB_PASSWORD}@${DB_HOST}:${DB_PORT}/${DB_NAME}
```

You can also set a one-off URL when running Alembic commands by editing `alembic.ini` or by setting the `--x` parameters as needed.
...
# Roll back to a specific revision (example)
alembic downgrade -1
```

4. Create a new migration revision (autogenerate from models/schema changes):

```bash
# Create a new revision file with an autogenerated diff
...
- Review autogenerated revisions before applying them; `--autogenerate` is a helper but can miss complex changes.
- Keep migration descriptions clear and concise.
- Use a dedicated database user for running migrations in production and ensure backups exist before applying destructive changes.
- If your `alembic/env.py` reads connection parameters from environment variables, make sure your `.env` (or shell environment) is loaded when running `alembic`.

If you need help bootstrapping Alembic to point to your Postgres URL, open `alembic.ini` and replace the `sqlalchemy.url` value with your database URL or modify `alembic/env.py` so it constructs the URL using the `.env` values.
...
Then, reload using the same command 
```bash
source ~/.zshrc
```
